...
const sections = [
  {
    ...
    links: [
      { label: "Quick start guide", href: "/support" },
      { label: "Dashboard overview", href: "/support" },
      { label: "Create your first project", href: "/sign-up" },
    ],
  },
  {
    icon: Terminal,
    ...
    links: [
      { label: "Authentication", href: "/support" },
      { label: "Endpoints overview", href: "/support" },
      { label: "Rate limits & quotas", href: "/support" },
    ],
  },
  {
    icon: Code2,
    ...
    links: [
      { label: "Node.js SDK", href: "/support" },
      { label: "Python SDK", href: "/support" },
      { label: "Go SDK", href: "/support" },
    ],
  },
  {
    icon: Shield,
    ...
    links: [
      ...
      { label: "Custom rules", href: "/support" },
      { label: "Challenge modes", href: "/support" },
    ],
  },
  {
    icon: Globe,
    ...
    links: [
      ...
      { label: "Alert configuration", href: "/support" },
      { label: "Detection logic", href: "/support" },
    ],
  },
  {
    icon: Zap,
    ...
    links: [
      ...
      { label: "Keyword management", href: "/support" },
      { label: "Webhook alerts", href: "/support" },
    ],
  },
];
    ...
    <div className="bg-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex justify-center mb-8">
            <div className="inline-flex items-center gap-2 border border-slate-200 rounded-full bg-white px-4 py-1.5 shadow-sm">
              <span className="bg-blue-600 text-white text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">Docs</span>
              <span className="text-sm text-slate-600">Always up to date</span>
            </div>
          </div>
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-slate-900">Documentation</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sections.map((section) => (
              <div key={section.title} className="rounded-2xl border border-slate-200 bg-white p-6 hover:border-blue-200 hover:shadow-md transition-all">
                ...
                <ul className="space-y-2">
                  {section.links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 font-medium transition-colors">
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="mb-12">
            ...
            <h2 className="mt-3 text-3xl md:text-4xl font-semibold text-slate-900">Step-by-step walkthroughs</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {guides.map((guide) => (
              <Link key={guide.title} href={guide.href} className="group flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 hover:-translate-y-0.5 hover:shadow-md transition-all">
                <div>
                  <span className="text-xs font-semibold uppercase tracking-wide text-blue-600">{guide.category}</span>
                  <h3 className="mt-1 text-base font-semibold text-slate-900 group-hover:text-slate-700">{guide.title}</h3>
      ...
      <section className="py-20 bg-white border-t border-slate-100">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Can&apos;t find what you&apos;re looking for?</h2>
          <p className="text-slate-500 text-lg mb-8">Our team is available around the clock to help you get unstuck.</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button className="bg-[#0a1120] hover:bg-[#1a253a] text-white px-8 py-5 rounded-md text-base font-medium" asChild>
              <Link href="/support">Contact support <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
            <Button variant="outline" className="border-slate-200 text-slate-600 hover:bg-slate-50 px-8 py-5 rounded-md text-base font-medium bg-transparent" asChild>
